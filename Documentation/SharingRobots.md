...
* Deselect the `Include dependencies` box on the bottom of the window that opens up. This should leave just the robot
  ...
  <img src="\Documentation\Img\SharingRobots\Export Image.png">
...
* In the import window, ensure that the robot prefab is selected and click `Import`
  <img src="\Documentation\Img\SharingRobots\Import Image.png">
* It's that simple! Your robot is now uploaded. To start playing with it, select it in the `GameManager` like any other robot and start playing. Don't forget that any robots you upload can still be edited as normal, so don't be afraid to change settings such as keybinds to your liking